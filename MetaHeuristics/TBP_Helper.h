	     ...
	     int MAX_ITER, int tExpire, int nExpire, int s, int t, int n)
{
	...
	theModel->IV_Frequency = (int **)malloc(sizeof(int *)*n);
	for(i = 0; i < n; i++) 
	{
		theModel->IV_Frequency[i] = (int *)malloc(sizeof(int)*n);
		for(j = 0; j < n; j++) theModel->IV_Frequency[i][j] = 0;
	}

	//Initialize and 0 out the matrix for Neighbor Frequency
	theModel->neighborFrequency = (int **)malloc(sizeof(int *)*n);
	for(i = 0; i < n; i++)
	{
		theModel->neighborFrequency[i] = (int *)malloc(sizeof(int)*n);
		for(j = 0; j < n; j++) theModel->neighborFrequency[i][j] = 0;
	}
}
...
{
	int i,j,k,l,a,b,x0,x1,y0,y1,aval,bval,temp,recency,tFound,nFound;
	double frequency;
	...
	for(i = 0; i < theModel->size; i++)
	{
		...
		//This is fine!!!!
		frequency = 0.0;
		frequency += theModel->IV_Frequency[a][bval];
		frequency += theModel->IV_Frequency[b][aval];

		if(x0b.sNeighbor != -1 && x0b.bNeighbor != theModel->n )
			frequency += theModel->neighborFrequency[x0b.sNeighbor][x0b.bNeighbor];
		if(x1b.sNeighbor != -1 && x1b.bNeighbor != theModel->n )
			frequency += theModel->neighborFrequency[x1b.sNeighbor][x1b.bNeighbor];
		if(y0a.sNeighbor != -1 && y0a.bNeighbor != theModel->n )
			frequency += theModel->neighborFrequency[y0a.sNeighbor][y0a.bNeighbor];
		if(y1a.sNeighbor != -1 && y1a.bNeighbor != theModel->n )
			frequency += theModel->neighborFrequency[y1a.sNeighbor][y1a.bNeighbor];

		frequency /= iter;

		//Did it! (Update everything, add stuff to lists etc)
		//printf("Recency is: %d\n",recency);
		if(recency <= 2 && frequency <= 0.1685)
		{
			...
			//Frequency update
			(theModel->IV_Frequency)[b][aval]++;
			(theModel->IV_Frequency)[a][bval]++;

			if(x0b.sNeighbor != -1 && x0b.bNeighbor != theModel->n )
				(theModel->neighborFrequency)[x0b.sNeighbor][x0b.bNeighbor]++;
			if(x1b.sNeighbor != -1 && x1b.bNeighbor != theModel->n )
				(theModel->neighborFrequency)[x1b.sNeighbor][x1b.bNeighbor]++;
			if(y0a.sNeighbor != -1 && y0a.bNeighbor != theModel->n )
				(theModel->neighborFrequency)[y0a.sNeighbor][y0a.bNeighbor]++;
			if(y1a.sNeighbor != -1 && y1a.bNeighbor != theModel->n )
				(theModel->neighborFrequency)[y1a.sNeighbor][y1a.bNeighbor]++;
	...
	,int MAX_ITER, int tExpire, int nExpire, int s, int t, int n)
{
	...
	int i,j,k,swapDex,a,b,temp,nThreads,**masterClones,size;
	double ***masterMatrix;
	Swap *paraPerm;
	...
	//Initialize and clone all threads into masterClones and masterMatrix
	masterClones = (int **)malloc(sizeof(int *)*nThreads);
	masterMatrix = (double ***)malloc(sizeof(double **)*nThreads);
	for(i = 0; i < nThreads; i++)
	{
		masterClones[i] = (int *)malloc(sizeof(int)*n);
		masterMatrix[i] = (double **)malloc(sizeof(double *)*n);
		for(j = 0; j < n; j++) 
		{
			masterClones[i][j] = root[j];
			masterMatrix[i][j] = (double *)malloc(sizeof(double)*n);
			for(k = 0; k < n; k++) masterMatrix[i][j][k] = matrix[j][k];
		}
	}
	
	//Construct theModel
	TB_Init(&theModel,root,rootVal,matrix,MAX_ITER,tExpire,nExpire,s,t,n);	
	paraPerm = theModel.permutations;
	size = theModel.size;
	...
	for(i = 0; i < theModel.MAX_ITER; i++)
	{
		...
		for(j = 0; j < size; j++)
		{
			paraPerm[j].value = fitness(masterClones[omp_get_thread_num()],
					            rootVal,
					            paraPerm[j].a,
						    paraPerm[j].b,
					            n,
					            masterMatrix[omp_get_thread_num()]);
		}
		...
		//Change the value of all the clones
		for(j = 0; j < nThreads; j++)
		{
			temp = masterClones[j][a];
			masterClones[j][a] = masterClones[j][b];
			masterClones[j][b] = temp;
		}
			
		//Change the value of the cost
		theModel.rootVal = theModel.permutations[swapDex].value;
		rootVal = theModel.rootVal;
